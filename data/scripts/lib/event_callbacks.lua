...
-- The fields within the assigned table allow saving arbitrary information
                ...
                repeat
                   results = {events[index].callback(unpack(args))}
                    ...
                    if info.returnValue then
                        if output == RETURNVALUE_NOERROR then break end
                        return output
                    end
                    -- We left the loop because we reached the end
                    if index == eventsCount then return unpack(results) end
                until true
